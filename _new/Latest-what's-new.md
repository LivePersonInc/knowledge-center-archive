...
## Proactive API 2.0
Proactive API 2.0 enables a scalable integration with the Conversational Cloud, moving away from the routing bot architecture. The proactive service will accept 10 api calls cumulative per second per account. The message send rate has been increased. The messages are sent at a rate up to 10 messages per second per channel per account and brands now have the ability to do batch calls.   
