...
{: .notice} Settings for security-related policies can be accessed and set by LivePerson employees only. 
...
{: .notice} LivePerson recommends setting a policy that enforces the use of at least two out of the three options mentioned above (e.g., Alpha character required + Number character required, or Alpha character required + Special character, etc.)
...
{: .notice} Email notifications will be sent to users when their passwords are approaching the selected expiration dates. You can send notifications to the user’s registered email address as well as to a specific email address.
...
{: .notice} To modify the default email address, contact your Account Manager or LivePerson Technical Support.
